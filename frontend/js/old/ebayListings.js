...
/**
 * Load eBay Listings Page
 */
async function loadEbayListings() {
  if (!document.getElementById("ebay-listings")) return;
  try {
    const res = await fetch("http://82.42.112.27:5000/api/ebay-listings");
    const listings = await res.json();
    document.getElementById("ebay-listings").innerHTML = formatEbayListings(listings);
  } catch (error) {
...
function updateVariantValues() {
  ...
  document.addEventListener('click', async (e) => {
    ...
    const listingId = e.target.dataset.itemId;
